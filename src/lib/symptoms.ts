...
export function formatSymptom(s: string): string {
  return s.replace(/_/g, " ").replace(/\(.*?\)/g, "").trim().replace(/\b\w/g, l => l.toUpperCase());
}

export function matchSymptoms(input: string): string[] {
  const words = input.toLowerCase().replace(/,/g, " ").split(/\s+/).filter(Boolean);
  const matched: string[] = [];
  for (const symptom of ALL_SYMPTOMS) {
    const parts = symptom.toLowerCase().replace(/_/g, " ").split(/\s+/);
    const symptomStr = symptom.toLowerCase().replace(/_/g, " ");
    if (words.some(w => symptomStr.includes(w)) || parts.some(p => words.includes(p))) {
